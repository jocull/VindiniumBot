...
namespace VindiniumCore.PathFinding
{
    ...
    public class PathFinder
    {
        ...
        public NodePath FindShortestPath(Node source, Node target, Func<Node, NodeStatus> statusFunc = null, int iterationLimit = 10000)
        {
            ...
            for (int iteration = 0; iteration < iterationLimit; iteration++)
            {
                ...
                foreach (Node adjacent in _NodeSet.GetNeighboringNodes(current))
                {
                    ...
                    if (adjacent != null)
                    {
                        ...
                        if (!_ClosedList.Contains(adjacent))
                        {
                            ...
                            if (!_OpenList.Contains(adjacent))
                            {
                                ...
                                if (status != null)
                                {
                                    adjacentPath.CostToThisPath = currentPath.CostToThisPath + status.Cost;
                                }
                                else
                                {
